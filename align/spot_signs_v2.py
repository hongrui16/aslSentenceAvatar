"""v2 (2026-09-19, dictionary route; v1 = spot_signs.py untouched). Changes: (1) the English stop list is cut down to words that are
NOT signs (articles, prepositions, copula); pronouns, negation, modals and wh-words ARE signs in ASL and are now spotted;
(2) per-gloss true-vs-shuffled caption-hit counts are written to the stats json, because for very frequent words the caption-hit
constraint is weak and each word must be vetted by its own lift before it enters the dictionary.
--- v1 docstring ---
...
STOP = set('a an the to of in on at for with and or but is are was were be been am its'.split())
...
def main():
    ...
    pg = {}  # per gloss: [n cos>=0.4, hit_true, hit_shuf, n cos>=0.6, hit_true, hit_shuf]
        ...
        for fp in sorted(glob.glob(os.path.join(a.bank_dir, 'bank_*.npz'))):
            ...
            for i in range(0, len(E), 200000):
                ...
                for k in range(len(cos)):
                    ...
                    if cos[k] >= 0.4:
                        q = pg.setdefault(uniq[g[k]], [0] * 6); q[0] += 1; q[1] += ht; q[2] += hs
                        if cos[k] >= 0.6: q[3] += 1; q[4] += ht; q[5] += hs
                    for c_, m_ in ths:
    ...
    res = {'n_segments': n_all, 'n_accepted_cos0.4': n_acc, 'thresholds': st, 'per_gloss_n_true_shuf_c04_c06': pg}; json.dump(res, open(a.stats, 'w'), indent=1); print(json.dumps(st, indent=1))
    for w in 'i we it not no have can what how this that who where why will my go'.split():
        if w in pg: q = pg[w]; print(f'{w}: cos>=0.4 n={q[0]} true={q[1]} shuf={q[2]} lift={q[1] / max(q[2], 1):.1f} | cos>=0.6 n={q[3]} true={q[4]} shuf={q[5]}')
