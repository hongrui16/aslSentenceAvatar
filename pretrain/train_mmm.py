"""Masked motion modeling over the 4 part-token streams (roadmap 4.1, stage 2): BERT-style transformer encoder that predicts
the masked tokens of every part (time-position masking, 30% of positions, of which 80% [MASK] / 10% random / 10% kept).
Token sequences come from tokenize_corpus.py. The encoder (without heads) is the init for the 4.2 segment encoder.
  python pretrain/train_mmm.py --tok_dir <dir> --train_list <ids> --val_list <ids> --steps 100000"""
...
def mask_tokens(x, valid, K, p=0.3):
    sel = (torch.rand(x.shape[:2], device=x.device) < p) & valid; r = torch.rand(x.shape[:2], device=x.device); inp = x.clone()
    inp[sel & (r < 0.8)] = K; rnd = sel & (r >= 0.8) & (r < 0.9); inp[rnd] = torch.randint(0, K, (int(rnd.sum()), 4), device=x.device)
    return inp, sel
...
def main():
    ...
    ap.add_argument('--d', type=int, default=512); ap.add_argument('--layers', type=int, default=8); ap.add_argument('--heads', type=int, default=8); ap.add_argument('--max_len', type=int, default=256); ap.add_argument('--mask_p', type=float, default=0.3)
    ...
    def run_batch(x, valid):
        x, valid = x.to(dev), valid.to(dev); inp, sel = mask_tokens(x, valid, a.K, a.mask_p); logits = model(inp, valid)
        loss = F.cross_entropy(logits[sel].reshape(-1, a.K), x[sel].reshape(-1)); acc = {p: (logits[sel][:, i].argmax(-1) == x[sel][:, i]).float().mean().item() for i, p in enumerate(PARTS)}; return loss, acc
    model.train()
    while step < a.steps:
        for x, valid in dl:
