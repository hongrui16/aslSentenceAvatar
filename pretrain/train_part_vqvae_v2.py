"""v2 (2026-09-17): per-part loss weights --w_hand/--w_face and FK finger weight --fk_w_finger; logs mpjpe_arm/finger. v1 = train_part_vqvae.py (untouched).
...
def losses(model, batch, fk, a, device):
    ...
    PW = {'body': 1.0, 'lhand': a.w_hand, 'rhand': a.w_hand, 'face': a.w_face}  # per-part loss weights (fingers fitted from low-res video are noisy)
    L_rec = sum(PW[p] * rec[p] for p in rec); L_vel = sum(PW[p] * vel[p] for p in vel); L_commit = sum(commit.values())
    with torch.autocast('cuda', enabled=False):
        J_gt = fk(batch['aa159'].to(device).float()); J = fk(assemble_aa159({p: o.float() for p, o in out.items()}))
        jw = torch.tensor([1.0] * 14 + [a.fk_w_finger] * 30, device=J.device)  # 44 joints = 14 body/arm + 15 + 15 fingers
        sq = ((J - J_gt) ** 2).sum(-1); L_fk = (sq * jw * mask.float()).sum() / (mask.sum() * jw.sum())
        d = sq.sqrt() * mask.float(); mpjpe_arm = 100 * d[..., :14].sum().item() / (mask.sum().item() * 14); mpjpe_fin = 100 * d[..., 14:].sum().item() / (mask.sum().item() * 30)
    total = L_rec + a.lambda_vel * L_vel + a.beta * L_commit + a.lambda_fk * L_fk
    return total, dict(rec=L_rec.item(), vel=L_vel.item(), commit=L_commit.item(), fk=L_fk.item(), mpjpe_cm=100 * (((J - J_gt) ** 2).sum(-1).sqrt() * mask.float()).sum().item() / (mask.sum().item() * 44), mpjpe_arm=mpjpe_arm, mpjpe_finger=mpjpe_fin, **{f'ppl_{p}': ppl[p].item() for p in PART_DIMS}, **{f'rec_{p}': rec[p].item() for p in PART_DIMS})
...
def main():
    ...
    ap.add_argument('--K', type=int, default=512); ap.add_argument('--width', type=int, default=256); ap.add_argument('--down', type=int, default=2); ap.add_argument('--beta', type=float, default=0.25); ap.add_argument('--lambda_vel', type=float, default=0.5); ap.add_argument('--lambda_fk', type=float, default=1.0); ap.add_argument('--w_hand', type=float, default=1.0); ap.add_argument('--w_face', type=float, default=1.0); ap.add_argument('--fk_w_finger', type=float, default=1.0)
