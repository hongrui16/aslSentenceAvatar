"""
ASL Avatar Diffusion Training — Voting Module
===============================================

Trains the voting-gated motion diffusion model. The LLM draft pseudo-gloss
is loaded from a pre-computed cache; the per-gloss voting gate is the only
new trainable component and is trained end-to-end via the diffusion noise
loss (no auxiliary gate loss).

Prerequisite:
    python tools/generate_llm_draft_gloss.py --modes train val test

Usage:
    accelerate launch trainMotionDiffusion_voting.py \
        --use_upper_body --use_rot6d --batch_size 100 --epochs 150
"""
...
class VotingDiffusionTrainer:
    ...
    def __init__(self, args):
        ...
        self.logging_dir = os.path.join(
            self.cfg.LOG_DIR, f"{self.cfg.PROJECT_NAME}_v1_voting",
        ...
        if self.debug:
            ...
            self.ckpt_dir = os.path.join(
                self.cfg.CKPT_DIR, f"{self.cfg.PROJECT_NAME}_v1_voting",
                self.cfg.DATASET_NAME, timestamp
            )
    ...
    def _build_components(self):
        ...
        self.logger.info("Building MotionDiffusionModelV1_Voting...")
        self.model = MotionDiffusionModelV1_Voting(self.cfg)
    ...
    def train(self):
        self.logger.info("=" * 60)
        self.logger.info("Starting Voting Diffusion Training")
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train ASL Voting Diffusion Model")
...
if __name__ == "__main__":
    args = parse_args()
    trainer = VotingDiffusionTrainer(args)
    trainer.train()
