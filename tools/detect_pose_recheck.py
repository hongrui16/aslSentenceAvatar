...
def process_batch(image_paths, images_dir, keypoints_dir, holistic,
    ...
    for idx, img_path in enumerate(image_paths):
        ...
        # Skip if at least one hand already detected; re-detect only when both are empty.
        if os.path.exists(out_json_path):
            try:
                with open(out_json_path, 'r') as _f:
                    _existing = json.load(_f)
                _lh_ok = len(_existing.get('left_hand_landmarks_3d',  [])) > 0
                _rh_ok = len(_existing.get('right_hand_landmarks_3d', [])) > 0
                if _lh_ok or _rh_ok:
                    skipped += 1
                    continue          # at least one hand present → keep
                # both hands empty → fall through and re-detect
            except Exception:
...
def process_directory(root_dir, asl_lex_csv=None, visualize=False,
    ...
    with mp_holistic.Holistic(
        ...
        min_detection_confidence=0.3,   # lowered from 0.5 for better hand recall
