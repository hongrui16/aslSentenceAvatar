...
# Graph topology
# ============================================================
# MediaPipe Pose edges (upper body only, node id = landmark id)
_BODY_EDGES = [
    # Face
    (0, 1), (1, 2), (2, 3), (3, 7),
    (0, 4), (4, 5), (5, 6), (6, 8),
    ...
    # Right arm
    (12, 14), (14, 16),
    # Pose hand tips (pinky/index/thumb knuckles)
    (15, 17), (15, 19), (15, 21),
    (16, 18), (16, 20), (16, 22),
    # Hips
    (11, 23), (12, 24),
...
# MediaPipe Hands edges (shared template for both hands, local ids 0-20)
_HAND_EDGES = [
    # Thumb
    (0, 1), (1, 2), (2, 3), (3, 4),
    # Index
...
# Node layout in the 67-node graph:
#   body     : 0  – 24  (UPPER_BODY_IDS)
#   left hand: 25 – 45  (LH_OFFSET + local id)
#   right hand:46 – 66  (RH_OFFSET + local id)
_LH_OFFSET = NUM_UPPER_LANDMARKS               # 25
_RH_OFFSET = NUM_UPPER_LANDMARKS + NUM_HAND_LANDMARKS  # 46


def build_adjacency() -> torch.Tensor:
    """
    Build normalised adjacency matrix A_hat = D^{-1/2} A D^{-1/2}
    for the 67-node body+hand graph (with self-loops).
    """
    N = TOTAL_LANDMARKS
    ...
    for i, j in _HAND_EDGES:
        A[_LH_OFFSET + i, _LH_OFFSET + j] = 1.0
        A[_LH_OFFSET + j, _LH_OFFSET + i] = 1.0

    # Right hand edges
    for i, j in _HAND_EDGES:
        A[_RH_OFFSET + i, _RH_OFFSET + j] = 1.0
        A[_RH_OFFSET + j, _RH_OFFSET + i] = 1.0

    # Cross-graph: body wrist → hand wrist
    # Left  wrist: pose id 15 (index 15) → lh node 25
    A[15, _LH_OFFSET] = A[_LH_OFFSET, 15] = 1.0
    # Right wrist: pose id 16 (index 16) → rh node 46
    A[16, _RH_OFFSET] = A[_RH_OFFSET, 16] = 1.0
...
class GraphConvLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adj: torch.Tensor) -> torch.Tensor:
        ...
        # x  : (B, T, N, C)
        # torch.matmul broadcasts: adj @ x over (B, T) dims → (B, T, N, C)
...
class HandshapeGNN(nn.Module):
    ...
    def __init__(self, adj: torch.Tensor,
                 ...
                 dropout:     float = 0.3):
        super().__init__()
        self.register_buffer('adj', adj)          # (67, 67), non-trainable
        self.num_nodes = adj.shape[0]             # 67
    ...
    def forward(self, x: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        x = x.view(B, T, self.num_nodes, 3)      # (B, T, 67, 3)

        # Spatial GCN
        x = self.gcn1(x, self.adj)               # (B, T, 67, gcn_hidden)
        x = self.gcn2(x, self.adj)               # (B, T, 67, gcn_out)
        x = self.gcn_norm(x)
